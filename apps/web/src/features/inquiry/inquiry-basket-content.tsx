...
export function InquiryBasketContent(): ReactElement {
  ...
  function handleRemove(lineId: string) {
    if (!items) return;
    const removedIndex = items.findIndex((item) => item.lineId === lineId);
    const nextItems = removeInquiryItem(lineId);
    const nextFocusItem = nextItems[Math.min(Math.max(removedIndex, 0), nextItems.length - 1)];
    pendingFocusTarget.current = nextFocusItem?.lineId ?? "empty";
    setItems(nextItems);
  }
          ...
          >
            <div>
              ...
              <strong aria-live="polite">
                {ar ? `${items.length} تهيئة محددة · الكمية الإجمالية ${totalQuantity}` : `${items.length} selected configurations · ${totalQuantity} total quantity`}
              </strong>
            </div>
        ...
        <Container size="wide">
          <div className="inquiry-preview-layout">
                    ...
                    >
                      ...
                      <div className="inquiry-preview-line__identity">
                        ...
                        <p className="inquiry-preview-line__options">
                          {ar ? "SKU" : "SKU"}: <bdi dir="ltr">{item.sku}</bdi> · {ar ? "المقاس" : "Size"}: {item.size || (ar ? "غير محدد" : "Not specified")} · {ar ? "الخيار" : "Variant"}: {item.variant || (ar ? "غير محدد" : "Not specified")}
                        </p>
                        <dl className="inquiry-preview-line__pricing">
                          <div>
                            <dt>{ar ? "سعر الوحدة" : "Unit price"}</dt>
                            <dd>{item.unitPriceSar ? formatSar(item.unitPriceSar, locale) : ar ? "السعر عند الطلب" : "Price on request"}</dd>
                          </div>
                          <div>
                            <dt>{ar ? "إجمالي البند" : "Line subtotal"}</dt>
                            <dd>{lineSubtotal ? formatSar(lineSubtotal, locale) : ar ? "السعر عند الطلب" : "Price on request"}</dd>
                          </div>
                        </dl>
                      </div>
                      <div className="inquiry-preview-line__controls">
                        <div className="inquiry-preview-quantity">
                          ...
                          <div>
                            <button
                              ...
                              onClick={() => setItems(updateInquiryItem(item.lineId, { quantity: item.quantity - 1 }))}
                            >−</button>
                            <motion.output
                              key={`${item.lineId}-${item.quantity}`}
                        ...
                        <label className="inquiry-preview-note">
                          ...
                          <input value={item.notes} maxLength={500} placeholder={ar ? "متطلب اختياري" : "Optional requirement"} onChange={(event) => setItems(updateInquiryItem(item.lineId, { notes: event.target.value }))} />
                        </label>
                        <button
                          type="button"
                          className="text-link"
                          ref={(node) => {
                            if (node) removeButtonRefs.current.set(item.lineId, node);
                            else removeButtonRefs.current.delete(item.lineId);
                          }}
            ...
            >
              ...
              <dl>
                <div><dt>{ar ? "التهيئات المحددة" : "Selected configurations"}</dt><dd><motion.output key={`products-${items.length}`} className="conversion-value" aria-live="polite" initial={reduceMotion ? false : { opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}>{items.length}</motion.output></dd></div>
                ...
                {pricing.allPriced && pricing.totalSar ? (
                  <div><dt>{ar ? "الإجمالي التقديري" : "Estimated total"}</dt><dd>{formatSar(pricing.totalSar, locale)}</dd></div>
                ) : null}
                {!pricing.allPriced && pricing.pricedSubtotalSar ? (
                  <div><dt>{ar ? "الإجمالي الفرعي للعناصر المسعرة" : "Priced items subtotal"}</dt><dd>{formatSar(pricing.pricedSubtotalSar, locale)}</dd></div>
                ) : null}
                {pricing.unpricedLineCount > 0 ? (
                  <div><dt>{ar ? "عناصر دون سعر منشور" : "Price on request"}</dt><dd>{ar ? `${pricing.unpricedLineCount} بند · ${pricing.unpricedQuantity} وحدة` : `${pricing.unpricedLineCount} line(s) · ${pricing.unpricedQuantity} unit(s)`}</dd></div>
                ) : null}
                {!pricing.allPriced ? (
                  <div><dt>{ar ? "إجمالي عرض السعر الكامل" : "Complete quotation total"}</dt><dd>{ar ? "قيد التسعير" : "Pending"}</dd></div>
                ) : null}
              </dl>
              <p>{ar ? "ستراجع روزا المنتجات المحددة والأسعار النهائية قبل إعداد عرض السعر." : "Rosa will review the selected configurations and final pricing before preparing the quotation."}</p>
