...
namespace {
    ...
    foreach ($widgetsManager->widgets as $widget) {
        ...
        if (! isset($homeExpected[$name])) {
            continue;
        }
        $homeWidgets[$name] = $widget;
    }

    if (array_keys($homeWidgets) !== array_keys($homeExpected)) {
        fail_test('Home Elementor widgets were not registered in the required order');
    }

    foreach ($homeExpected as $name => $expectedControls) {
        $widget = $homeWidgets[$name];
    ...
    fwrite(STDOUT, "PASS: Elementor integration and Home widget contracts\n");
}
