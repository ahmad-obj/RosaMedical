...
! grep -Fq 'rosa_is_latest_home_page' "$TEMPLATE" || fail 'finished-template Home must not suppress the shared CTA'
...
! grep -Fq 'latest-rosa-home.css' "$THEME/functions.php" || fail 'superseded latest Home stylesheet must not be enqueued'
! grep -Fq 'latest-rosa-home.js' "$THEME/functions.php" || fail 'superseded latest Home script must not be enqueued'
...
$expectedHome = ["rosa-home-hero","rosa-home-who","rosa-home-featured","rosa-home-feature-banner","rosa-home-latest","rosa-home-promotions","rosa-home-why","rosa-home-proof","rosa-home-evidence"];
foreach ($targets as [$path, $pageType, $locale]) {
    ...
    if ($pageType === "home" && (string) get_post_meta($id, "_rosa_elementor_home_parity_version", true) !== "2") {
        WP_CLI::error("Finished-template Home parity version 2 missing for {$path}");
    }
    ...
    if ($pageType === "home") {
        ...
        $root = is_array($elements) ? ($elements[0] ?? []) : [];
        ...
        if ($actual !== $expectedHome) WP_CLI::error("Finished-template Home widget order mismatch for {$path}");
        if ((string)($root["settings"]["css_classes"] ?? "") !== "rosa-elementor-root") WP_CLI::error("Home root class drifted for {$path}");
    }
}
WP_CLI::success("Six Rosa Elementor authoring documents verified, including finished-template Home parity v2.");
'

printf 'PASS: Elementor authoring protected-shell, finished-template Home parity and runtime contract\n'